...
import { Music, Image as ImageIcon, Play, Pause, Shuffle, Loader2, Square, Zap, Type, Film, Sliders, Maximize, Move } from 'lucide-react';
...
function App() {
  ...
  const [vizOffsetY, setVizOffsetY] = useState(90); 
  ...
  const { 
    togglePlay, stop, isPlaying, audioElement, audioContext, sourceNode, analyser,
  ...
  const getVizDefaults = (type: string) => {
    const bottom = ['spectrum', 'wave', 'bars_3d'];
    if (bottom.includes(type)) return { x: 50, y: 95 };
    return { x: 50, y: 50 };
  };
  ...
  const handleRandomize = () => {
    ...
    setVizScale(0.8 + Math.random() * 0.6);
    setVizRotation(Math.floor(Math.random() * 360));
    ...
    // Background random zoom occasionally
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      const recorder = new MediaRecorder(new MediaStream([...canvasStream.getVideoTracks(), ...dest.stream.getAudioTracks()]), { mimeType: 'video/webm;codecs=vp9' });
      const chunks: Blob[] = [];
      recorder.ondataavailable = (e) => { if (e.data.size > 0) chunks.push(e.data); };
      recorder.onstop = () => {
        const url = URL.createObjectURL(new Blob(chunks, { type: 'video/webm' }));
        const a = document.createElement('a'); a.href = url; a.download = `muzyv_${audioFile.name}.webm`; a.click();
        setRendering(false); restoreAudioAfterRecording();
      };
      resetAudioForRecording(); recorder.start();
      if (audioContext.state === 'suspended') await audioContext.resume();
      await audioElement.play();
      audioElement.addEventListener('ended', () => { recorder.stop(); }, { once: true });
    } catch (e: unknown) { console.error(e); setRendering(false); }
  };
      ...
      <main className="main-preview">
        <div className="canvas-wrapper">
          <Visualizer ref={canvasRef} analyser={analyser} bgUrl={bgUrl} bgType={bgType} vizType={vizType}
            bgZoom={bgZoom} bgOffsetX={bgOffsetX} bgOffsetY={bgOffsetY}
            ...
            smartSensitivity={smartSensitivity} vizScale={vizScale} vizRotation={vizRotation}
